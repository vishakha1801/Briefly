...
void main() {
    ...
    // Parameters for the animated centers in polar coordinates (Unrolled for strict WebGL 1.0 array indexing compatibility)
    float centers[7];
    centers[0] = originalCenters[0] + 0.5 * sin(uTime / 20.0 + uOffsets[0]);
    centers[1] = originalCenters[1] + 0.5 * sin(uTime / 20.0 + uOffsets[1]);
    centers[2] = originalCenters[2] + 0.5 * sin(uTime / 20.0 + uOffsets[2]);
    centers[3] = originalCenters[3] + 0.5 * sin(uTime / 20.0 + uOffsets[3]);
    centers[4] = originalCenters[4] + 0.5 * sin(uTime / 20.0 + uOffsets[4]);
    centers[5] = originalCenters[5] + 0.5 * sin(uTime / 20.0 + uOffsets[5]);
    centers[6] = originalCenters[6] + 0.5 * sin(uTime / 20.0 + uOffsets[6]);

    float a, b;
    vec4 ovalColor;

    // Check if the pixel is inside any of the ovals (Unrolled for strict WebGL 1.0 loop and array-access compatibility)
    float ovalNoise;
    float distTheta;

    // Oval 0
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[0] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[0]), min(abs(theta + 2.0 * PI - centers[0]), abs(theta - 2.0 * PI - centers[0])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, false, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }

    // Oval 1
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[1] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[1]), min(abs(theta + 2.0 * PI - centers[1]), abs(theta - 2.0 * PI - centers[1])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, true, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }

    // Oval 2
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[2] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[2]), min(abs(theta + 2.0 * PI - centers[2]), abs(theta - 2.0 * PI - centers[2])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, false, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }
    ...
    // Oval 3
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[3] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[3]), min(abs(theta + 2.0 * PI - centers[3]), abs(theta - 2.0 * PI - centers[3])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, true, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }
    ...
    // Oval 4
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[4] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[4]), min(abs(theta + 2.0 * PI - centers[4]), abs(theta - 2.0 * PI - centers[4])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, false, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }
    ...
    // Oval 5
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[5] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[5]), min(abs(theta + 2.0 * PI - centers[5]), abs(theta - 2.0 * PI - centers[5])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, true, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }
    ...
    // Oval 6
    ovalNoise = texture(uPerlinTexture, vec2(mod(centers[6] + uTime * 0.05, 1.0), 0.5)).r;
    a = 0.5 + ovalNoise * 0.3;
    b = ovalNoise * mix(3.5, 2.5, uInputVolume);
    distTheta = min(abs(theta - centers[6]), min(abs(theta + 2.0 * PI - centers[6]), abs(theta - 2.0 * PI - centers[6])));
    if (drawOval(vec2(distTheta, radius), vec2(0.0, 0.0), a, b, false, 0.6, ovalColor)) {
        color.rgb = mix(color.rgb, ovalColor.rgb, ovalColor.a);
        color.a = max(color.a, ovalColor.a);
    }
